...
__ARCH__ ?= $(shell uname -m | sed 's/x86_64/x86/' \
			| sed 's/arm.*/arm/' \
			| sed 's/aarch64/arm64/' \
...
# outdated
INCLUDES := -I$(OUTPUT) -I../../libbpf/include/uapi -I./include -I$(LIBBLAZESYM_INC)
CFLAGS := -g -Wall -O2
CXXFLAGS := -std=c++20 -g -Wall -O2
CXX_LDFLAGS := -stdlib=libc++ -lc++ -lc++abi
ALL_LDFLAGS := $(LDFLAGS) $(EXTRA_LDFLAGS) $(CXX_LDFLAGS)

APPS = memsnoop fdsnoop
...
$(call allow-override,CXX,$(CROSS_COMPILE)cxx)
...
$(OUTPUT)/%.bpf.o: %.bpf.c $(LIBBPF_OBJ) $(wildcard %.h) | $(OUTPUT) $(BPFTOOL)
	$(call msg,BPF,$@)
	$(Q)$(CLANG) $(CFLAGS) -target bpf -D__TARGET_ARCH_$(__ARCH__)	\
...
$(OUTPUT)/%.o: %.cpp $(wildcard %.h) | $(OUTPUT)
	$(call msg,CXX,$@)
	$(Q)$(CXX) $(INCLUDES) $(CXXFLAGS) -c $(filter %.cpp,$^) -o $@ $(CXX_LDFLAGS)

$(APPS): %: $(OUTPUT)/%.o $(LIBBLAZESYM_OBJ) $(LIBBPF_OBJ) | $(OUTPUT)
	$(call msg,CXX,$@)
	$(Q)$(CXX) $(INCLUDES) $^ $(ALL_LDFLAGS) -lelf -lz -o $@
